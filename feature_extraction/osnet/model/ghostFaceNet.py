...
class ModifiedGDC(nn.Module):
    def __init__(self, image_size, in_chs, num_classes, dropout, emb=512): # dropout implementation is in the original code but not in the paper
        ...
        self.conv_dw = nn.Conv2d(in_chs, in_chs, kernel_size=(10,4), groups=in_chs, bias=False)
...
class GhostFaceNetsV2(nn.Module):
    def __init__(self, cfgs=None, image_size=256, num_classes=0, width=1.0, channels=3, dropout=0.2, block=GhostBottleneckV2,
        ...
        if cfgs == None:
            self.cfgs =  [
                #卷积核大小(k)，扩展因子(t)，输出通道数(c)，SE模块比率(SE)，步长(s)
                # k, t, c, SE, s 
                [[3,  16,  16, 0, 1]],
                [[3,  48,  24, 0, 2]],
                [[3,  72,  24, 0, 1]],
        ...
        #添加批量归一化层，以稳定学习过程并加速训练。
        self.bn1 = nn.BatchNorm2d(output_channel)
        #添加 PReLU 激活函数，这是一个参数化的ReLU，其参数可以在训练过程中学习。
        self.act1 = nn.PReLU()
        #将当前输出通道数设置为下一层的输入通道数
        ...
        for cfg in self.cfgs:
            layers = []
            #在每个配置内遍历，每个配置项包含卷积核大小 (k), 扩展因子 (exp_size), 输出通道数 (c), SE模块比例 (se_ratio), 和步长 (s)。
            for k, exp_size, c, se_ratio, s in cfg:
                #计算每层的输出通道数和隐藏通道数，并确保它们可以被4整除
                output_channel = _make_divisible(c * width, 4)
                hidden_channel = _make_divisible(exp_size * width, 4)
                #使用 GhostBottleneckV2 块创建网络层，并添加到层列表中
