...
class ChannelGate(nn.Module):
    ...
    def forward(self, x):
        ...
        # 进行全局平均池化，输出每个通道的 1x1 特征图
        x = self.fc1(x)
        # 第一个全连接层，使用 1x1 卷积实现，通道数减少为原来的 1/reduction。
        if self.norm1 is not None:
            x = self.norm1(x)
        x = self.relu(x)
        # 经过 ReLU 激活函数
        x = self.fc2(x)
        # 第二个全连接层，使用 1x1 卷积实现，将通道数变为 num_gates。
...
class OSBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, IN=False, bottleneck_reduction=4, **kwargs):
        ...
        self.cbam = CBAM(mid_channels)  # Add CBAM module with correct mid_channels
...
def init_pretrained_weights(model, key=''):
    ...
    # cached_file = "/home/zyq/zyq/reserch/osnet/osnetv4/model_best.pth"
    # cached_file = "/home/zyq/zyq/LightMBN/model-best.pth"
